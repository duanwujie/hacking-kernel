...
extern double   MaxDrawDownPercentage        = 50;       // Percent of portion for max drawdown level. ×î´ó»Ø³·ÂÊ
...
double      hDDStart;
double 		PbMax,PbMin;/*×î´óÀûÈóºÍ×îÐ¡ÀûÈó*/
double		PhMax,PhMin;/*EA×î´óÀûÈóºÍ×îÐ¡ÀûÈó*/
...
{	CS="Waiting for next tick .";      // To display comments while testing, simply use CS = .... and
	...
	//²ÎÊý¼ì²é£¬ÉèÖÃ´íÁË²ÎÊý½«Ê¹ÓÃÄ¬ÈÏ²ÎÊý
...
{	
	...
	CbT=CbB+CbS;/*×ÜµÄ¶©µ¥ÊýÁ¿£¬²»°üÀ¨EA*/
	LbT=LbB+LbS;/* total slots exclude EA */
	Pb=ND(Pb+BCb,2);
	ChT=ChB+ChS;/* EA×ÜµÄ¶©µ¥ÊýÁ¿*/
	LhT=LhB+LhS;
	Ph=ND(Ph+BCh,2);
	...
	//+-----------------------------------------------------------------+
	if(LbT>0)/*·ÇEAµÄ×ÜÊÖÊý>0*/
	{	BEb=ND(BEb/LbT,Digits);/*¼ÆËãÆ½¾ù½»Ò×ÉÌÊÖÐø·Ñ*/
		if(BCa<0)
			BEb-=ND(BCa/PipVal2/(LbB-LbS),Digits);/* ½»Ò×ÉÌµ¹¸øÇ®µÄÇé¿ö*/
		if(Pb>PbMax||PbMax==0)
			PbMax=Pb;
		if(Pb<PbMin||PbMin==0)
			PbMin=Pb;
		if(!TradesOpen)/*EAÃ»ÓÐ¿ªµ¥£¬Ö»ÊÇÍ³¼Æ*/
		{	
	...
	{	
        Trend=ForceMarketCond;/*ÊÖ¶¯Ö¸¶¨Ç÷ÊÆ·½Ïò*/
	...
	{	string sCalcTP="Set New TP: ";
		...
		BCaL=BCa;
		BEbL=BEb;
		BasePips=ND(Lot*LotMult*GridTP*(CbT+CbC)/(LbB-LbS),Digits);
		if(CbB>0)
		{	if(ForceTPPips>0)
			...
			if(BCa<0)
			{	double TPAdj=ND(BCa/PipVal2/(LbB-LbS),Digits);
				NewTP-=TPAdj;
		...
		{	if(ForceTPPips>0)
			...
			if(BCa<0)
			{	TPAdj=ND(BCa/PipVal2/(LbB-LbS),Digits);
				NewTP-=TPAdj;
				sCalcTP=sCalcTP+" -BC: ("+DTS(-TPAdj,Digits)+") :";
			}
	...
	{	double TPa,PhPips;
		...
		{	BEa=ND(((BEb*LbT-(BEh+hAsk-hBid)*LhT)/(LbT-LhT)),Digits);
			if(nLots>0)TPa=ND(BEa+TargetPips,Digits);
			else if(nLots<0)TPa=ND(BEa-TargetPips,Digits);
		}
		else
		{	//BEa=BEb-ND(Ph/PipVal2/(LbB-LbS),Digits);
		}
	...
	{	if(CbT==0)
		...
		if(CbT>0)
		{	if((PbPips+PhPips*LhT/LbT)<0&&SLb>0)SLb=0;
			if(SLb>0&&(PbPips+PhPips*LhT/LbT)<=SLb)
			...
			{	double TPbMP=ND(PbTarget*ProfitSet,Digits);
				if((PbPips+PhPips*LhT/LbT)>PbTarget*ProfitSet)SLb=ND(PbTarget*ProfitSet,Digits);
			}
	...
	{	if(displayOverlay)
		{	color Colour;
			...
			if(SLb>0)
			{	if(CbB>0)DrawLabel("B3VPrSL",BEb+SLb*Pip,190);
				else if(CbS>0)DrawLabel("B3VPrSL",BEb-SLb*Pip,190);
			}
